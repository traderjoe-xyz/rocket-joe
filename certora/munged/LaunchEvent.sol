...
contract LaunchEvent is Ownable {
    ...
    struct UserAllocation {
        uint256 allocation;
        bool hasWithdrawnPair; // boolean for checking if the user has already withdrawn LP tokens
    }
    ...
    uint256 public floorPrice; // floor price (min price we'd like) in AVAX for a single token
    ...
    uint256 public rJoePerAvax; // amount of rJOE to deposit certain amount of avax = amountAvax * rJoePerAvax
    IWAVAX public WAVAX;    // avax wrapper
    IERC20 public token;    // the token to be auctioned

    IJoeRouter02 public router; // uniswap like router
    IJoeFactory public factory; // uniswap like factory
    IRocketJoeFactory public rocketJoeFactory; // Creates individual LaunchEvent contracts. Also sets rJoePerAvax.
    ...
    uint256 private avaxAllocated; // total avax in the pool
    uint256 private tokenAllocated; // total tokens in the pool
    uint256 private lpSupply;   // minted LP tokens

    uint256 private tokenReserve;   // total tokens in this contract
    ...
    modifier withdrawable() {
        require(
            currentPhase() == Phase.PhaseOne ||         // G: call currentPhase() once and store in a variable
                currentPhase() == Phase.PhaseTwo,
    ...
    function initialize(
        ...
        require(
            _fixedWithdrawPenalty < 5e11, // isn't it 20% = 2e11?
    ...
    function createPair() external atPhase(Phase.PhaseThree) {
        ...
        // check the uniswap price in a pool 
    ...
    function emergencyWithdraw() external {
        ...
        if (msg.sender == issuer) {
            token.transfer(issuer, token.balanceOf(issuer)); // tokenReserve!?
        }
    }
