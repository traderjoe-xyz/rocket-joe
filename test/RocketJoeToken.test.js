...
describe("Rocket Joe Staking Contract", function () {
  ...
  describe("Should allow transfer from and to authorized address", function () {
    it("should allow minting", async function () {
      await this.rJOE.connect(this.owner).mint(this.alice.address, ethers.utils.parseEther("1"))
    });

    it("should allow burning", async function () {
      await this.rJOE.connect(this.owner).mint(this.alice.address, ethers.utils.parseEther("1"))

      await this.rJOE.connect(this.alice).burn(ethers.utils.parseEther("1"))
    });

    it("should allow transferring from owner", async function () {
      await this.rJOE.connect(this.owner).mint(this.owner.address, ethers.utils.parseEther("1"))

      await this.rJOE.connect(this.owner).transfer(this.alice.address, ethers.utils.parseEther("1"))
    });
    ...
    it("should allow transferring to a launch event", async function () {
      ...
      await this.rJOE.connect(this.owner).transfer(LaunchEvent.address, ethers.utils.parseEther("1"))
    });

    it("should revert if user tries to transfer token to a non-authorized address", async function () {
      await this.rJOE.connect(this.owner).mint(this.alice.address, ethers.utils.parseEther("1"))
      await this.rJOE.connect(this.owner).mint(this.bob.address, ethers.utils.parseEther("1"))

      await expect(
          this.rJOE.connect(this.alice).transfer(this.bob.address, ethers.utils.parseEther("1"))
      ).to.be.revertedWith("RocketJoeToken: can't send token")

      await expect(
          this.rJOE.connect(this.bob).transfer(this.alice.address, ethers.utils.parseEther("1"))
      ).to.be.revertedWith("RocketJoeToken: can't send token")
    });
  });
