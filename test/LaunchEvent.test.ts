...
describe("Launch Event Contract", function () {
  ...
  beforeEach(async function () {
    ...
    await this.rJOE.connect(this.dev).mint(this.dev.address, "1000000000000000000000000")
    await this.rJOE2.connect(this.dev).mint(this.dev.address, "1000000000000000000000000")
  ...
  describe("Initialising the contract", function() {
    ...
    it("should revert initialisation if issuer is not set", async function () {
      await expect(
        this.RocketFactory.createRJLaunchEvent(ethers.constants.AddressZero, Math.floor(Date.now() / 1000), this.rJOE2.address, 100, 1, 1, 1, 100, 10000, 60, 120)
    ...
    it("should revert initialisation if withdraw penalty gradient is too high", async function () {
      await expect(
		this.RocketFactory.createRJLaunchEvent(this.alice.address, Math.floor(Date.now() / 1000) + 60, this.rJOE2.address, 100, 1, 2893519, 1, 100, 10000, 60, 120)
    ...
    it("should revert initialisation if fixed withdraw penalty is too high", async function () {
      await expect(
		this.RocketFactory.createRJLaunchEvent(this.alice.address, Math.floor(Date.now() / 1000) + 60, this.rJOE2.address, 100, 1, 2893517, 6e11, 100, 10000, 60, 120)
    ...
    it("should revert initialisation if max allocation is greater than min", async function () {
	  await expect(
		this.RocketFactory.createRJLaunchEvent(this.alice.address, Math.floor(Date.now() / 1000) + 60, this.rJOE2.address, 100, 1, 2893517, 4e11, 10001, 10000, 60, 120)
      ).to.be.revertedWith("LaunchEvent: Max allocation needs to be greater than min")
	})

    it("should revert initialisation if user timelock is too long", async function () {
	  await expect(
		this.RocketFactory.createRJLaunchEvent(this.alice.address, Math.floor(Date.now() / 1000) + 60, this.rJOE2.address, 100, 1, 2893517, 4e11, 5000, 10000, 60*60*24*8, 120)
    ...
    it("should deploy with correct paramaters", async function () {
		this.RocketFactory.createRJLaunchEvent(this.alice.address, Math.floor(Date.now() / 1000) + 60, this.rJOE2.address, 100, 1, 2893517, 4e11, 5000, 10000, 60*60*24*7-1, 60*60*24*8)
    })
